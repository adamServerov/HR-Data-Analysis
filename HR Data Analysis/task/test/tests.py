...
class AggTest(StageTest):
    ...
    def check(self, reply: str, attach):
        ...
        for key in user_dict.keys():
            ...
            if not isinstance(curr_user_dict, dict):
                return CheckResult.wrong(f"The item `{curr_user_dict}` should be a dictionary!")
            for key_curr in curr_user_dict.keys():
                if key_curr not in curr_answer_dict.keys():
                    return CheckResult.wrong(f'Output should not contain \"{key_curr}\" as a key for the employment status')
